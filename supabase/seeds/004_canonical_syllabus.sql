-- ============================================================
-- Seed 004: Canonical Syllabus — Banking + SSC CGL 2027
-- ONE canonical topic per concept. Many-to-many exam mapping
-- via topic_exam_map. No duplicate topic records.
--
-- Hierarchy: Subject → Chapter → Topic
-- Subjects use exam_type = 'both' for shared, specific for exclusive.
--
-- IMPORTANT: Run MIGRATION 014 in SQL Editor FIRST before this.
-- IMPORTANT: Run this in Supabase SQL Editor (not db push).
-- ============================================================

-- ── BLOCK 1: Wipe old data + insert subjects + chapters + topics ──────────────
DO $$
DECLARE
  v_user_id uuid := (SELECT user_id FROM public.profiles ORDER BY created_at DESC LIMIT 1);
...
SELECT
  ...
  COUNT(DISTINCT tem.id) FILTER (WHERE tem.exam_type = 'banking') AS banking_mapped,
  COUNT(DISTINCT tem.id) FILTER (WHERE tem.exam_type = 'ssc')     AS ssc_mapped
FROM subjects s
...
LEFT JOIN topic_exam_map tem ON tem.topic_id = t.id
WHERE s.user_id = (SELECT user_id FROM profiles WHERE id = 'f84506bd-63ed-427c-9e8f-80da90f030b5')
  AND s.deleted_at IS NULL
GROUP BY s.name, s.exam_type, s.sort_order
